...
class LDAMLoss_CB(nn.Module):
    ...
    def __init__(self, cls_num_list, max_m=0.5, s=30, keep_prob=0.4, epoch=1, beta=0.999, reg=0.01):
    ...
    def forward(self, x, target):
        ...
        loss = F.cross_entropy(self.s * output, target,  reduce=False)
        loss = torch.mean(mask_ * loss)*self.reg+F.cross_entropy(self.s * output, target, weight=self.weights)
