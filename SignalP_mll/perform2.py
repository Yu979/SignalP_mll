...
def evaluate_crf(X, label):
    ...
    for i, (input, target) in enumerate(test_loader):
        ...
        o1, o_aa= model(input)
        o_aa = model.module.crf.decode_based_on_prob(o_aa, padding=False).permute(1, 0, 2)
        output.extend(o1.cpu().detach().numpy())
        output_aa.extend(o_aa.cpu().detach().numpy())
        labels_test.extend(target_test.cpu().detach().numpy())
        labels_test_aa.extend(target_aa.cpu().detach().numpy())
...
def aaTest(output_aa_origin, labels_test_aa_origin, labels_test_origin, testType):
    ...
    indexes_1 = torch.where(output_aa == 0)
    ...
    output_aa = output_aa.reshape(-1, 70).argmax(dim=1).detach().cpu().numpy()

    indexes_ = np.where(labels_test_aa == 1)
    labels_test_aa[indexes_] = 100

    indexes_1 = np.where(labels_test_aa == 0)
    ...
    labels_test_aa = torch.tensor(labels_test_aa).reshape(-1,70).argmax(dim=1).detach().cpu().numpy()

    # TP
    p = np.sum(np.equal(output_aa, labels_test_aa))
    s = output_aa.shape[0]
...
if __name__ == '__main__':
    ...
    # # 读取模型
    model = torch.load("./best_models/LDAM_CB_crf.pth")
    ...
    for key in X_test_cls.keys():
        ...
        y_pred, output_aa, labels_test, labels_test_aa = evaluate_crf(X_test_cls[key], labels_test_cls[key])

        # y_pred, labels_test = loader_test(X_test_cls[key], labels_test_cls[key])
        print()
        print("Test SP Type:")
        print("SP VS NO_SP")
