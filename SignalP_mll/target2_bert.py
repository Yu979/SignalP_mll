...
from embedding_creator.msa_create_embedding import *
from embedding_creator.probert_create_full_embedding import *
...
def createTrainTestData(data_path='./data/data_list.txt', label_path="./data/target_list.txt"
              ...
              maxlen=max_len, test_split=0.2, seed=1993
              ):
    # 初始化
    data_list=[]
    label_list=[]
    ...
    features = np.concatenate((trans_data_msa_in_batches(raw_data),
                               trans_data_pro_in_batches(raw_data).reshape(-1, max_len * embedding_feature_dim_pro)),
                              axis=1)
    ...
    np.random.seed(seed)
    np.random.shuffle(kingdom_train)
    np.random.seed(seed)
    np.random.shuffle(aa_train)
    np.random.seed(seed)
    ...
    np.random.seed(seed)
    np.random.shuffle(kingdom_val)
    np.random.seed(seed)
    np.random.shuffle(aa_val)
    np.random.seed(seed)
...
def create_test_files_cls():
    ...
    feature_list = {'EUKARYA': [], 'ARCHAEA': [], 'POSITIVE': [], 'NEGATIVE': []}
    ...
    filename_feature={'EUKARYA':"./embedding/feature_EUKARYA.npy",
                'ARCHAEA':"./embedding/feature_ARCHAEA.npy",
                'POSITIVE':"./embedding/feature_POSITIVE.npy",
                'NEGATIVE':"./embedding/feature_NEGATIVE.npy"}
    outf1={}
    outf2={}
    outf3={}
    outf4={}
    ...
    return filename_data, filename_target, filename_kingdom, filename_aa, filename_feature
...
def createTestData(data_path='./test_data/data_list.txt', label_path="./test_data/target_list.txt",
                   ...
                   test_path2="./pro_embedding/test_full_feature.npy"
                   ):
    # 初始化
    data_list = []
    label_list = []
    ...
    features = np.concatenate((trans_data_msa_in_batches(raw_data, path=test_path),
                               trans_data_pro_in_batches(raw_data, path=test_path2).reshape(-1, max_len*embedding_feature_dim_pro)
                               ), axis=1)
...
if __name__ == '__main__':
    ...
    lstm_config = {'dropout_rate': 0.2, 'input_dim': 1024+4,
    ...
    if (args.model_arch=="CNN"):
        ...
        model = BertModel(config, lstm_config)
