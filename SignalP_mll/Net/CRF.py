...
class CRF(nn.Module):
    ...
    def decode_based_on_prob(self, emissions: torch.Tensor,
               mask: Optional[torch.ByteTensor] = None, padding=True, reduce=False) -> List[List[int]]:
    ...
    def _compute_score_reweight(
        ...
        for i in range(1, seq_length):
            # Transition score to next tag, only added if next timestep is valid (mask == 1)
            # shape: (batch_size,)
            tag_list = tags[i].detach().cpu().numpy().tolist()
            reweight_factor = torch.tensor([self.reweight_ratio[tag] for tag in tag_list]).to(device)
            score += self.transitions[tags[i - 1], tags[i]] * mask[i] * reweight_factor

            # Emission score for next tag, only added if next timestep is valid (mask == 1)
            # shape: (batch_size,)
            score += emissions[i, torch.arange(batch_size), tags[i]] * mask[i] * reweight_factor
    ...
    def _compute_normalizer_reweight(
        ...
        # shape: (batch_size, num_tags)
        score += self.end_transitions
        score = torch.mul(score, torch.tensor([0.07, 0.93, 0.00]).to(device)).cuda()
